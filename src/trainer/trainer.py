...
class Trainer(ABC):
    ...
    def __call__(
        ...
        for epoch in range(epochs):
            tr_loss = self.__train_model(TS_SET, tr_times, tr_memory, desc=f'{epoch + 1}/{epochs} training')
            ts_loss = self.__evaluate_model(TS_SET, desc=f'{epoch + 1}/{epochs} evaluation')
            log.info(f'epoch: {epoch + 1:>4}/{epochs} - tr_loss: {tr_loss:>10.6f} - ts_loss: {ts_loss:>10.6f}')

        tr_accuracy, ts_accuracy = self.__compute_accuracy(TR_SET, TS_SET)

        self.__write_record_in_file(
            tr_loss,
            ts_loss,
            (sum(tr_times) / len(tr_times)) if len(tr_times) > 0 else 0,
            (sum(tr_memory) / len(tr_memory)) if len(tr_memory) > 0 else 0,
            tr_accuracy=tr_accuracy,
            ts_accuracy=ts_accuracy,
        )
        return self.model

    def __train_model(self, TR_SET, tr_times, tr_memory, desc):
        tr_loss_sum, tr_loss_len = 0, 0
        self.model.train()
        start_time = time.time()

        for TR_X_MB, TR_Y_MB in tqdm(TR_SET, desc=desc):
            tr_loss_sum += self.train_mb(TR_X_MB, TR_Y_MB)
            tr_loss_len += 1

        tr_times.append(time.time() - start_time)
        tr_memory.append(
        ...
        self.lrs.step()
        return tr_loss_sum / tr_loss_len

    def __evaluate_model(self, TS_SET, desc):
        ts_loss_sum, ts_loss_len = 0, 0
        self.model.eval()
        for X_MB, Y_MB in tqdm(TS_SET, desc=desc):
            with torch.autocast(device_type=self.device, dtype=self.precision):
                ...
                ts_loss_len += 1
        return ts_loss_sum / ts_loss_len

    def __compute_accuracy(self, TR_SET, TS_SET):
